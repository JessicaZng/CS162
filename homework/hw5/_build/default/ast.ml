...
(* 
[x] 1. Start with the basics: implement the typing rules for integer, nil, and cons values. 
  For type checking to be interesting, you will need values of different types.
[x] 2. Move on to the arithmetic, inequality, equality, and if-then-else rules. 
  Make sure that a type error is raised when e.g. 
    [x] adding a number to a list or 
    [x] comparing two expression of different types.
[ ] 3. Next, implement the T-Var, T-Lambda, and T-App rules.
[ ] 4. Now implement the rules for the list operations.
[ ] 5. Lastly, implement the T-Let rule. 
*)
